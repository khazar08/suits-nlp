...
def scrape_all(
    ...
    if seasons:
        ...
        print(f"Filtered to seasons {seasons}: {len(episodes)} episodes\n")
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--seasons", type=int, nargs="+", default=None,
                        help="Scrape only these season numbers, e.g. --seasons 1 2")
    args = parser.parse_args()
    scrape_all(Path(args.out), delay=args.delay, seasons=args.seasons)
