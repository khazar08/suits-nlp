...
def run(data_dir: Path, skip_scrape: bool, gap_seconds: float) -> pd.DataFrame:
    ...
    if skip_scrape:
        ...
        transcript_df = scrape_all(data_dir)
...
def main():
    ...
    run(Path(args.data_dir), skip_scrape=args.skip_scrape, gap_seconds=args.gap)
