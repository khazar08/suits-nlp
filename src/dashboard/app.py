...
def tab_power():
    ...
    for season in sel_seasons[1:]:
        first_ep = df[df["season"] == season]["ep_label"].iloc[0] if not df[df["season"] == season].empty else None
        if first_ep:
            fig.add_vline(x=first_ep, line_dash="dot", line_color="rgba(255,255,255,0.15)",
                          annotation_text=f"S{season}", annotation_font_color="#aaa")
...
def _demo_prediction(ep_id: str, dom) -> None:
    ...
    st.caption("*(Demo: using current influence scores — train a model for real predictions)*")
    _show_prediction(ranked, ep_id, dom)
